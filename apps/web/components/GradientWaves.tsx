...
const hexToRgb = (hex: string): [number, number, number] => {
  ...
  return [parseInt(result[1]??'00', 16) / 255, parseInt(result[2]??'00', 16) / 255, parseInt(result[3]??'00', 16) / 255];
};
...
const GradientWaves: React.FC<GradientWavesProps> = ({
  ...
  useEffect(() => {
    ...
    const canInteract = mouseInteraction && !isMobile;
    enableMouseRef.current = canInteract;
    ...
    if (u.uEnableMouse) u.uEnableMouse.value = canInteract;
